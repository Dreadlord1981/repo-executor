use std::{env, fs, io::{self, Read, Write}, net::TcpStream, path::{Path, PathBuf}, process::{Command, Stdio}};
use anyhow::anyhow;
...
impl<'a> Export<'a> {
	...
	pub fn deploy(&self) -> Result<bool, anyhow::Error> {
		...
		if dist || create {
			local_path = self.create_dist()?
		}
		else {
			local_path = self.create_export()?
		};
		...
		for entry in WalkDir::new(local_path.clone()).into_iter().filter_map(|e| e.ok()) {
			...
			if meta_data.is_dir() {
				...
				scp.write_all(content.as_bytes())?;
			}
		}
	...
	fn create_export(&self) -> Result<PathBuf, anyhow::Error> {
		...
		if head_ref != revision_file_server.admin.revision {
			...
			if success && temp_list.exists() {
				...
				let cmd = format!("git archive -o {} HEAD $(git diff --name-only --diff-filter=d {} HEAD)", file_path.clone().to_str().unwrap(), revision_file_server.admin.revision);

				let cmd_output = if cfg!(target_os = "windows") {

					Command::new("powershell")
						.args(["-Command", cmd.clone().as_str()])
						.stdout(Stdio::inherit())
						.output().unwrap()
				} else {
					Command::new("sh")
				...
				if success {
					...
					fs::remove_file(temp_list.clone()).unwrap();
